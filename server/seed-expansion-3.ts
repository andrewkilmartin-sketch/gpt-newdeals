// Seed script to add final batch of activities to reach 500 total
// Run with: npx tsx server/seed-expansion-3.ts

import { db } from './db';
import { activities } from '../shared/schema';
import { expansionActivities3 } from './data/activities-expansion-3';
import { eq } from 'drizzle-orm';

async function seedExpansionActivities3() {
  console.log('Adding final batch of expansion activities to database...');
  console.log(`Source: Web research from National Trust, Woodland Trust, Science Buddies, parenting sites`);
  
  let added = 0;
  let skipped = 0;
  
  for (const activity of expansionActivities3) {
    try {
...
seedExpansionActivities3().catch((err) => {
