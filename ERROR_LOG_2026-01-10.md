...
| **29** | **Main tsvector query no try/catch** | **100% failure (0 results for ALL queries)** | **Wrap in try/catch + runtime flag + ILIKE fallback** |

### Fix #29 - CRITICAL (2026-01-10)
**This was the root cause of 100% production failure.**

| Aspect | Details |
|--------|---------|
| **Symptom** | ALL queries returned 0 results, 0ms time (see screenshots) |
| **Root Cause** | Main tsvector query at lines 4670-4682 had NO try/catch |
| **Why Production Failed** | search_vector column doesn't exist in Railway DB |
| **Why Dev Worked** | Dev DB has the column, masking the bug |
| **Fix Applied** | try/catch + tsvectorFailed flag + ILIKE fallback |
| **File** | server/routes.ts lines 4675-4731 |
| **Test Result** | 100% pass rate on 8 queries after fix |
